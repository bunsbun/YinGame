...
AYCharacter::AYCharacter()
{
    ...
    PunchCollisionBox->SetupAttachment(GetMesh(), FName("whip_04"));// ѕрив€зываем к мечу

	PunchCollisionBox->SetCollisionProfileName(TEXT("Pawn"));
    PunchCollisionBox->SetGenerateOverlapEvents(false);

    PunchDamage = 20.0f; // ”станавливаем значение урона
}
...
void AYCharacter::MoveForward(float Value)
{
	...
	//¬ы€сните, какой путь €вл€етс€ "пр€мым", и запишите, что игрок хочет двигатьс€ в этом направлении
